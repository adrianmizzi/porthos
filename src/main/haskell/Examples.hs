...
groupPay yy recipient = allOf (userCommits yy) `followedBy`
                          ifThenElse (countC allCommitments .==. length yy)
                            ...
                            cancelAll end)
  where
    userCommits = map (\x -> onUserCommit ("pay", EUR, txFilter x) end (onTimeout 100 end))
...
crowdFunding2 recipient (x, y) target = both (campaignX, campaignY) `followedBy` closeCampaign
  where
    campaignX = repeatCommit ("fundX", xType, isCommitTo recipient)
                  (onTimeout 100 end)
    campaignY = repeatCommit ("fundY", yType, isCommitTo recipient)
                  (onTimeout 100 end)
...
propSale (seller, property) buyer deposit balance notary =
            onUserCommit ("commitProperty", Property{ref="xyz"}, isCommitTo buyer .&. isCommitBy seller .&. isAsset property)
              doBuyerCommit
              (onTimeout 10 end)
  where
...
t2 = both (c1, c2) .>>>. fireEvent "ready" c3
  where
    ...
    c2 = fireEvent "starting c2"
           (onUserCommit ("c2", EUR, isCommitBy alice .&. isCommitTo bob) end
             (onTimeout 200 end))
    c3 = onUserCommit ("c3", GBP, isCommitBy charlie .&. isCommitTo bob) (releaseAll end)
          (onTimeout 200 (cancelAll end))
